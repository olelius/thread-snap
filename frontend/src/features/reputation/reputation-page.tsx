...
import { Activity, Beaker, CalendarClock, CarFront, ChartNoAxesCombined, Check, ChevronLeft, ChevronRight, CircleAlert, ExternalLink, FileCheck2, Gauge, Images, Loader2, Play, Plus, RefreshCw, Rocket, RotateCcw, ScanSearch, Settings2, Trash2 } from 'lucide-react'
...
type PublishPreview = {
  ...
  project_group_changed_count: number
  warning?: string
}
...
function ScopePanel({ query, adapterStatus, adapterMessage }: { query: ReturnType<typeof useQuery<ReputationScope>>; adapterStatus?: ReputationCapabilities['real_adapter_status']; adapterMessage?: string }) {
  ...
  const projectGroupMutation = useMutation({
    mutationFn: ({ vehicle, projectGroup }: { vehicle: ReputationScopeVehicle; projectGroup: string }) => api<ReputationScope>(`/reputation/scope/vehicles/${encodeURIComponent(vehicle.id)}`, {
      method: 'PATCH',
      body: JSON.stringify({ revision: query.data?.revision, project_group: projectGroup }),
    }),
    onSuccess: (value) => { refreshScopeState(value); toast.success('项目组归属已保存', { description: '发布变更后将进入后续巡检范围快照。' }) },
    onError: (error) => toast.error('项目组归属保存失败', { description: errorMessage(error) }),
  })
  ...
  return <div className='flex h-full min-h-0 flex-col gap-3'>
    ...
    <Dialog open={publishOpen} onOpenChange={setPublishOpen}><DialogContent className='sm:max-w-2xl'><DialogHeader><DialogTitle className='flex items-center gap-2'><Rocket className='size-5 text-primary' />确认发布范围变更</DialogTitle><DialogDescription>本次将冻结 {activeVehicles.length} 款启用车型和 {verified} 项已验证映射；后续计划使用新范围，既有批次保持不变。</DialogDescription></DialogHeader><div className='grid grid-cols-2 gap-3 rounded-lg border bg-muted/20 p-3 text-sm sm:grid-cols-5'><div><div className='text-xs text-muted-foreground'>新增</div><div className='mt-1 font-semibold'>{publishPreview?.added_count ?? 0}</div></div><div><div className='text-xs text-muted-foreground'>停用</div><div className='mt-1 font-semibold'>{publishPreview?.disabled_count ?? 0}</div></div><div><div className='text-xs text-muted-foreground'>角色/顺序</div><div className='mt-1 font-semibold'>{publishPreview?.role_changed_count ?? 0}</div></div><div><div className='text-xs text-muted-foreground'>名称/映射</div><div className='mt-1 font-semibold'>{(publishPreview?.mapping_changed_count ?? 0) + (publishPreview?.identity_changed_count ?? 0)}</div></div><div><div className='text-xs text-muted-foreground'>项目组</div><div className='mt-1 font-semibold'>{publishPreview?.project_group_changed_count ?? 0}</div></div></div><div className='max-h-56 overflow-auto rounded-lg border p-3'><div className='grid grid-cols-2 gap-x-4 gap-y-2 text-sm'>{activeVehicles.map((vehicle) => <div key={vehicle.id} className='flex items-center justify-between gap-2'><span className='truncate'>{vehicle.vehicle_name}</span><Badge variant='outline' className='max-w-36 truncate font-normal'>{vehicle.project_group}</Badge></div>)}</div></div><DialogFooter><Button variant='outline' onClick={() => setPublishOpen(false)}>返回核对</Button><Button disabled={publishMutation.isPending} onClick={() => publishMutation.mutate()}>{publishMutation.isPending ? <Loader2 className='size-4 animate-spin' /> : <Rocket className='size-4' />}确认发布</Button></DialogFooter></DialogContent></Dialog>
  </div>
}

function ProjectGroupInput({ vehicle, pending, onSave }: { vehicle: ReputationScopeVehicle; pending: boolean; onSave: (projectGroup: string) => void }) {
  const [value, setValue] = useState(vehicle.project_group)
  useEffect(() => setValue(vehicle.project_group), [vehicle.project_group])
  const normalized = value.trim()
  const dirty = normalized !== vehicle.project_group
  return <div className='flex min-w-48 items-center gap-1.5'><Input value={value} maxLength={80} disabled={pending} className='h-8 min-w-0' aria-label={`${vehicle.vehicle_name}项目组归属`} onChange={(event) => setValue(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && dirty && normalized) onSave(normalized); if (event.key === 'Escape') setValue(vehicle.project_group) }} /><Button type='button' variant='ghost' size='icon' className={`size-8 shrink-0 ${dirty ? '' : 'invisible'}`} aria-label={`保存${vehicle.vehicle_name}项目组归属`} disabled={!dirty || !normalized || pending} onClick={() => onSave(normalized)}>{pending && dirty ? <Loader2 className='size-4 animate-spin' /> : <Check className='size-4' />}</Button></div>
}
