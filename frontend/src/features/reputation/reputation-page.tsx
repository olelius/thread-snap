    ...
    <Card className='flex h-full min-h-0 flex-col overflow-hidden border-border/70 bg-card/90 py-0 shadow-sm backdrop-blur'>
      ...
      <div className='min-h-0 flex-1 overflow-auto'>
        <Table className='min-w-[980px]'>
          <TableHeader><TableRow className='bg-muted/35'><TableHead>巡检编号</TableHead><TableHead>类型</TableHead><TableHead>平台与范围</TableHead><TableHead>状态</TableHead><TableHead>处理进度</TableHead><TableHead>证据完整度</TableHead><TableHead>完成时间</TableHead><TableHead className='text-right'>操作</TableHead></TableRow></TableHeader>
          <TableBody>
            {items.length ? items.map((run) => {
              ...
              return <TableRow key={run.id} tabIndex={0} className='cursor-pointer transition-colors hover:bg-primary/[0.035] focus-visible:bg-primary/[0.06] focus-visible:outline-none' onClick={() => onOpen(run.id)} onKeyDown={(event) => { if (event.key === 'Enter') onOpen(run.id) }}>
                ...
                <TableCell className='text-right'><Button variant='ghost' size='sm' onClick={(event) => { event.stopPropagation(); onOpen(run.id) }}>查看</Button></TableCell>
              </TableRow>
