          ...
          <TableBody>
            {items.length ? items.map((run) => {
              ...
              return <TableRow key={run.id} tabIndex={0} className='cursor-pointer transition-colors hover:bg-primary/[0.035] focus-visible:bg-primary/[0.06] focus-visible:outline-none' onClick={() => onOpen(run.id)} onKeyDown={(event) => { if (event.key === 'Enter') onOpen(run.id) }}>
                ...
                <TableCell><div className='flex items-center gap-1.5 text-sm'><FileCheck2 className={`size-4 ${run.complete_evidence_count === run.required_evidence_count ? 'text-emerald-500' : 'text-amber-500'}`} />{run.required_evidence_count ? `${run.complete_evidence_count}/${run.required_evidence_count}` : '历史未要求'}</div></TableCell>
