...
import { reputationMappingStatus, reputationMissingMappingIds, reputationValidationTargetIds } from '@/features/reputation/reputation-validation'
...
type PublishPreview = {
  ...
  publishable_vehicle_count: number
...
function ScopePanel({ query, platforms, adapterStatus, adapterMessage }: { query: ReturnType<typeof useQuery<ReputationScope>>; platforms?: ReputationCapabilities['reputation_platforms']; adapterStatus?: ReputationCapabilities['real_adapter_status']; adapterMessage?: string }) {
  ...
  const verified = activeVehicles.filter((item) => reputationMappingStatus(item.mappings[platformCode], platformCode, selectedPlatform?.validation_contract_version) === 'verified').length
  ...
  return <div className='flex h-full min-h-0 flex-col gap-3'>
    ...
    {!scope.initialized ? <Card><CardHeader><CardTitle className='text-base'>车型范围尚未初始化</CardTitle><CardDescription>{scope.message} 初始化属于一次性运维动作，避免浏览器上传真实业务清单。</CardDescription></CardHeader><CardContent className='rounded-lg bg-muted/45 p-4 font-mono text-xs'>threadsnap reputation-init --file &lt;UTF-8-CSV&gt;</CardContent></Card> : <div className='flex min-h-0 flex-1 flex-col gap-3'>
      ...
      <div className='flex flex-wrap items-center justify-between gap-3 px-4 py-3'>
        <div className='flex min-w-0 items-center gap-3'>
          ...
          <div className='min-w-0'>
            ...
            <div className='mt-0.5 text-xs text-muted-foreground'>维护车型与平台映射；验证通过后发布生效，指标请到巡检批次查看。</div>
          </div>
        </div>
      ...
      <div className='min-h-0 flex-1 overflow-auto'>
        <Table className='min-w-[1080px]' aria-label='车型与映射'>
          <TableHeader className='sticky top-0 z-10 bg-card shadow-[0_1px_0_hsl(var(--border))]'><TableRow className='bg-muted/35 hover:bg-muted/35'><TableHead className='w-24 pl-4'>角色顺序</TableHead><TableHead>车系</TableHead><TableHead>车型</TableHead><TableHead className='min-w-36'>项目组归属</TableHead><TableHead>{selectedPlatformName}展示名</TableHead><TableHead>验证状态</TableHead><TableHead className='whitespace-nowrap' title='最近一次成功验证的北京时间，不是最近巡检时间'>最近通过时间</TableHead><TableHead className='text-center'>验证证据</TableHead><TableHead className='text-center'>平台页面</TableHead><TableHead className='pr-4 text-right'>操作</TableHead></TableRow></TableHeader>
          <TableBody>{displayVehicles.map((vehicle) => {
            const mapping = vehicle.mappings[platformCode]
            const validationState = reputationMappingStatus(mapping, platformCode, selectedPlatform?.validation_contract_version)
            const statusTone = validationState === 'verified' ? 'success' : validationState === 'stale' ? 'partial_success' : validationState === 'failed' ? 'failed' : 'unknown'
            const displayName = validationState === 'verified' ? mapping?.actual_name || mapping?.platform_display_name : mapping?.platform_display_name
            return <TableRow key={vehicle.id} data-scope-vehicle={vehicle.id} className={vehicle.enabled ? undefined : 'bg-muted/20 text-muted-foreground'}>
              <TableCell className='w-24 pl-4'><ReputationRoleLabel role={vehicle.role} position={vehicle.role_order} /></TableCell>
              <TableCell>{vehicle.series_name}</TableCell>
              <TableCell className='font-medium'>{vehicle.vehicle_name}</TableCell>
              <TableCell>{vehicle.project_group}</TableCell>
              <TableCell>{displayName || '—'}</TableCell>
              <TableCell><span title={validationState === 'stale' ? '当前映射内容或验证规则已变化，请重新验证。' : undefined}>{vehicle.enabled ? <StatusBadge value={statusTone} label={mappingStatusName(validationState)} /> : <Badge variant='outline'>已停用</Badge>}</span>{vehicle.enabled && validationState === 'failed' && mapping?.validation_error && <div className='mt-1 max-w-44 text-[11px] text-destructive'>{mapping.validation_error}</div>}</TableCell>
              <TableCell className='whitespace-nowrap text-sm tabular-nums'>{formatDate(mapping?.validated_at)}</TableCell>
              <TableCell className='text-center'>{urlOnly ? <span className='text-xs text-muted-foreground'>URL 模式</span> : mapping?.validation_attempt_id ? <Button asChild variant='ghost' size='icon'><a href={`/api/v1/reputation/mapping-validations/attempts/${mapping.validation_attempt_id}/metric`} target='_blank' rel='noreferrer' aria-label='查看上次验证证据'><Images className='size-4' /></a></Button> : '—'}</TableCell>
              <TableCell className='text-center'>{mapping?.platform_url ? <Button asChild variant='ghost' size='icon'><a href={mapping.platform_url} target='_blank' rel='noreferrer' aria-label='打开平台页面'><ExternalLink className='size-4' /></a></Button> : '—'}</TableCell>
              <TableCell className='pr-4 text-right'><ScopeVehicleAction vehicle={vehicle} editPending={saveVehicleMutation.isPending} restorePending={restoreMutation.isPending} onEdit={() => openVehicleDialog(vehicle)} onRemove={() => setRemoveTarget(vehicle)} onRestore={() => restoreMutation.mutate(vehicle)} /></TableCell>
            </TableRow>
          })}</TableBody>
        </Table>
      </div>
...
function mappingStatusName(value: ReturnType<typeof reputationMappingStatus>) { return { verified: '验证通过', unverified: '待验证', failed: '验证失败', missing: '缺少映射', stale: '需重新验证' }[value] }
