...
function EvidencePanel({ results }: { results: ReputationResult[] }) {
  ...
  return <div className='h-full overflow-auto pr-1'><div className='grid gap-4 md:grid-cols-2 2xl:grid-cols-3'>{evidence.map((result, index) => <motion.article key={result.id} initial={reduceMotion ? false : { opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.18, delay: reduceMotion ? 0 : Math.min(index * 0.025, 0.2) }}><Card className='group overflow-hidden border-border/70 py-0 transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md motion-reduce:transform-none'><a href={result.evidence!.full_page_url} target='_blank' rel='noreferrer' className='relative block overflow-hidden border-b bg-muted/35'><img src={result.evidence!.metric_region_url} alt={`${result.vehicle_name} 指标区域证据`} className='aspect-[4.45/1] w-full object-cover transition-transform duration-300 group-hover:scale-[1.015] motion-reduce:transform-none' /><span className='absolute right-2 bottom-2 rounded-md bg-background/90 px-2 py-1 text-[11px] shadow-sm backdrop-blur'>打开完整原页 <ArrowRight className='ml-1 inline size-3' /></span></a><CardHeader className='pb-2'><div className='flex items-start justify-between gap-2'><div><CardTitle className='text-base'>{result.vehicle_name}</CardTitle><CardDescription>{result.series_name} · {result.platform_name}</CardDescription></div><Badge variant='outline'>{result.role === 'focus' ? '重点车型' : '竞品'}</Badge></div></CardHeader><CardContent className='pb-4'><div className='grid grid-cols-3 gap-2 text-xs'><EvidenceMetric label='口碑分' value={result.metrics.score.raw} /><EvidenceMetric label='排名' value={result.metrics.rank.raw} /><EvidenceMetric label='口碑量' value={result.metrics.volume.raw} /></div><div className='mt-3 truncate font-mono text-[10px] text-muted-foreground' title={result.evidence!.full_page_sha256}>SHA-256 · {result.evidence!.full_page_sha256}</div></CardContent></Card></motion.article>)}</div>{!evidence.length && <Card className='grid h-full min-h-64 place-items-center'><CardContent className='text-center'><ImageIcon className='mx-auto mb-2 size-7 text-muted-foreground/50' /><div className='font-medium'>本批次没有页面证据</div></CardContent></Card>}</div>
}
