...
function RankingPanel({ results, onViewEvidence }: { results: ReputationResult[]; onViewEvidence: (result: ReputationResult) => void }) {
  ...
  return <Card className='flex h-full min-h-0 flex-col overflow-hidden border-border/70 bg-card/90 py-0 shadow-sm'><div className='min-h-0 flex-1 overflow-auto'><Table className='min-w-max'><TableHeader><TableRow className='bg-muted/35'><TableHead rowSpan={2} className='reputation-sticky-head sticky left-0 z-30 w-24 pl-4'>角色</TableHead><TableHead rowSpan={2} className='reputation-sticky-head sticky left-24 z-30 min-w-40'>车型</TableHead>{platforms.map(([code, name]) => <TableHead key={code} colSpan={6} className='border-l text-center font-semibold'>{name}</TableHead>)}</TableRow><TableRow className='bg-muted/25'>{platforms.flatMap(([code]) => [<TableHead key={`${code}-score`} className='border-l text-right'>口碑分</TableHead>, <TableHead key={`${code}-rank`} className='text-right'>排名</TableHead>, <TableHead key={`${code}-volume`} className='text-right'>口碑量</TableHead>, <TableHead key={`${code}-reviews`} className='text-right'>评价篇数</TableHead>, <TableHead key={`${code}-negative`} className='text-right'>差评率</TableHead>, <TableHead key={`${code}-state`} className='min-w-32'>状态/证据</TableHead>])}</TableRow></TableHeader><TableBody>{vehicles.map((vehicle) => <TableRow key={vehicle.vehicle_id} className='group'><TableCell className='sticky left-0 z-20 bg-card pl-4 transition-colors group-hover:bg-muted/50'><ReputationRoleLabel role={vehicle.role} position={vehicle.vehicle_position} /></TableCell><TableCell className='sticky left-24 z-20 bg-card transition-colors group-hover:bg-muted/50'><div className='font-medium'>{vehicle.vehicle_name}</div><div className='mt-1 text-xs text-muted-foreground'>{vehicle.series_name}</div></TableCell>{platforms.flatMap(([code]) => {
