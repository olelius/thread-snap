...
function RankingPanel({ results, onViewEvidence }: { results: ReputationResult[]; onViewEvidence: (result: ReputationResult) => void }) {
  return <Card className='flex h-full min-h-0 flex-col overflow-hidden border-border/70 bg-card/90 py-0 shadow-sm'><div className='min-h-0 flex-1 overflow-auto'><Table className='min-w-[1320px]'><TableHeader><TableRow className='bg-muted/35'><TableHead className='reputation-sticky-head sticky left-0 z-20 w-24 pl-4'>角色</TableHead><TableHead className='reputation-sticky-head sticky left-24 z-20 min-w-40'>车型</TableHead><TableHead>平台</TableHead><TableHead className='text-right'>口碑分</TableHead><TableHead className='text-right'>排名</TableHead><TableHead className='text-right'>口碑量</TableHead><TableHead className='text-right'>口碑评价篇数</TableHead><TableHead className='text-right'>差评率</TableHead><TableHead>状态</TableHead><TableHead className='pr-4 text-right'>证据</TableHead></TableRow></TableHeader><TableBody>{results.map((result) => <TableRow key={result.id} className='group'><TableCell className='sticky left-0 z-10 bg-card pl-4 transition-colors group-hover:bg-muted/50'><ReputationRoleLabel role={result.role} position={result.vehicle_position} /></TableCell><TableCell className='sticky left-24 z-10 bg-card transition-colors group-hover:bg-muted/50'><div className='font-medium'>{result.vehicle_name}</div><div className='mt-1 text-xs text-muted-foreground'>{result.series_name}</div></TableCell><TableCell>{result.platform_name}</TableCell><TableCell className='text-right'><MetricCell metric={result.metrics.score} /></TableCell><TableCell className='text-right'><MetricCell metric={result.metrics.rank} inverseLabel /></TableCell><TableCell className='text-right'><MetricCell metric={result.metrics.volume} /></TableCell><TableCell className='text-right'><MetricCell metric={result.metrics.review_article_count ?? historicalMetric()} /></TableCell><TableCell className='text-right'><MetricCell metric={result.metrics.negative_rate ?? historicalMetric()} /></TableCell><TableCell>{result.status === 'success' ? <StatusBadge value='success' label='成功' /> : <div><StatusBadge value='failed' label='异常' /><div className='mt-1 max-w-52 text-[11px] text-muted-foreground'>{result.error_message}</div></div>}</TableCell><TableCell className='pr-4 text-right'>{result.evidence ? <Button variant='ghost' size='sm' onClick={() => onViewEvidence(result)}><ImageIcon className='size-4' />查看截图</Button> : <span className='text-xs text-muted-foreground'>{result.evidence_required ? '缺失' : '历史未要求'}</span>}</TableCell></TableRow>)}</TableBody></Table></div><div className='shrink-0 border-t bg-card/95 px-4 py-3 text-xs text-muted-foreground'>颜色表达业务方向：口碑分、口碑量和口碑评价篇数升高为绿色；排名数字和差评率下降代表改善，同样为绿色。颜色始终配合箭头和文字。</div></Card>
}
