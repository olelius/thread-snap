...
export function reputationMetricColumns(platformCode: string, capabilities?: Pick<ReputationCapabilities, 'reputation_platforms'>): Array<[ReputationMetricKey, string]> {
  ...
  return keys.filter((key): key is ReputationMetricKey => Object.prototype.hasOwnProperty.call(labels, key) && !(key === 'negative_rate' && ['autohome', 'yiche'].includes(platformCode))).map((key) => [key, platformCode === 'autohome' && key === 'circle_content_count' ? '论坛帖子总数' : labels[key]])
}
