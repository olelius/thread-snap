  ...
  return <div className='space-y-4 xl:flex xl:h-full xl:min-h-0 xl:flex-col xl:gap-4 xl:space-y-0'>
    ...
    <div className='max-h-[min(65svh,680px)] overflow-auto rounded-xl border bg-card/90 xl:min-h-0 xl:max-h-none xl:flex-1' data-list-viewport='circles'>
      <Table className='min-w-[1180px]'>
        ...
        <TableBody>{rows.map((row, index) => { const baseline = baselineById.get(row.id); const isNew = !baseline; const platformDirty = isNew || row.platform_code !== baseline.platform_code; const vehicleDirty = isNew || row.vehicle_name !== baseline.vehicle_name || row.vehicle_id !== baseline.vehicle_id; const urlDirty = isNew || row.url !== baseline.url; const autoDirty = Boolean(baseline) && row.auto_enabled !== baseline?.auto_enabled; const rowDirty = isNew || platformDirty || vehicleDirty || urlDirty || autoDirty; return <TableRow key={row.id || `new-${index}`} data-dirty={rowDirty || undefined} className={cn(rowDirty && 'bg-amber-500/[0.035]')}>
