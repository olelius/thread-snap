...
function PlanPanel({ onDirtyChange }: { onDirtyChange: (dirty: boolean) => void }) {
  ...
  const save = useMutation({ mutationFn: () => api<ExtractionPlan>('/extraction-plan', { method: 'PUT', body: JSON.stringify({ revision: draft?.revision, rules: draft?.rules.map(({ id, name, platform_quantities, circle_ids }) => ({ id, name, platform_quantities, circle_ids })), nodes: draft?.nodes.map(({ id, weekdays, time, enabled, rule_id }) => ({ id, weekdays, time, enabled, rule_id })) }) }), onSuccess: (value) => { setDraft(structuredClone(value)); setDirty(false); onDirtyChange(false); client.setQueryData(['extraction-plan'], value); toast.success('提取计划已保存') }, onError: (error) => toast.error('保存失败', { description: errorMessage(error) }) })
  ...
  return <div className='space-y-5'>
    <div className='flex flex-wrap items-center justify-between gap-3'><div><h2 className='text-lg font-semibold'>自动提取规则</h2><p className='text-sm text-muted-foreground'>每条规则明确选择平台及其圈子，并为已选平台设置统一的每圈有效结果目标数。</p></div><div className='flex gap-2'><Badge variant='outline'>计划版本 {draft.revision}</Badge><Button variant='outline' onClick={() => update({ ...draft, rules: [...draft.rules, { id: crypto.randomUUID(), name: `新规则 ${draft.rules.length + 1}`, version: 1, platform_quantities: {}, circle_ids: [], archived: false, updated_at: new Date().toISOString() }] })}><Plus className='size-4' />新建规则</Button><Button disabled={save.isPending} onClick={() => save.mutate()}>{save.isPending ? <Loader2 className='size-4 animate-spin' /> : <Save className='size-4' />}保存当前标签</Button></div></div>
