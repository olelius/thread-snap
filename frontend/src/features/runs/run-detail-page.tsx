...
import { ArrowLeft, CircleStop, ChevronLeft, ChevronRight, Copy, Download, ExternalLink, KeyRound, RefreshCw, Trash2 } from 'lucide-react'
...
export function RunDetailPage() {
  ...
  const posts = useQuery({
    queryKey: ['posts', runId, { ...search, title: debouncedTitle, circle: debouncedCircle }],
  ...
  const detail = useQuery({ queryKey: ['post', runId, search.post], queryFn: () => api<Post>(`/runs/${runId}/posts/${search.post}`), enabled: Boolean(search.post) })
  ...
  function patch(values: Partial<SearchState>) {
    navigate({ to: '/runs/$runId', params: { runId }, search: (previous) => ({ ...previous, ...values }), replace: true })
  }

  const retry = useMutation({ mutationFn: () => api<Run>(`/runs/${runId}/retry`, { method: 'POST', headers: { 'Idempotency-Key': crypto.randomUUID() } }), onSuccess: (value) => { toast.success('失败项已重新提交', { description: `新批次 ${value.number}` }); client.invalidateQueries({ queryKey: ['runs'] }) }, onError: (error) => toast.error('重新提取失败', { description: errorMessage(error) }) })
  ...
  const endAuthWait = useMutation({ mutationFn: () => api<Run>(`/runs/${runId}/end-auth-wait`, { method: 'POST' }), onSuccess: (value) => { client.setQueryData(['run', runId], value); client.invalidateQueries({ queryKey: ['runs'] }); toast.success('本次提取已结束') }, onError: (error) => toast.error('结束提取失败', { description: errorMessage(error) }) })
  const deleteRun = useMutation({ mutationFn: () => api<{ message: string }>(`/runs/${runId}`, { method: 'DELETE' }), onSuccess: () => { client.removeQueries({ queryKey: ['run', runId] }); client.invalidateQueries({ queryKey: ['runs'] }); toast.success('批次及关联快照已永久删除'); navigate({ to: '/runs', search: emptyRunsSearch }) }, onError: (error) => toast.error('删除批次失败', { description: errorMessage(error) }) })
  ...
  function moveToPost(postId: string | undefined, position: number) {
    if (!postId) return
    const page = Math.ceil(position / (search.pageSize ?? 50))
    patch({ post: postId, page })
  }
  ...
  return (
    <div className='space-y-6'>
      ...
      <Sheet open={Boolean(search.post)} onOpenChange={(open) => !open && patch({ post: undefined })}><SheetContent className='w-full overflow-y-auto p-0 sm:max-w-[58vw]'><SheetHeader className='sticky top-0 z-10 border-b bg-background/90 p-6 backdrop-blur'><div className='flex items-start justify-between gap-4 pr-8'><div><SheetTitle>{detail.data?.title || '帖子快照详情'}</SheetTitle><SheetDescription className='mt-1'>数据库快照，不会在打开时重新访问平台。{navigation.data && ` 当前为筛选结果第 ${navigation.data.position} / ${navigation.data.total} 条。`}</SheetDescription></div><div className='flex gap-1'><Button variant='outline' size='icon' disabled={!navigation.data?.previous_id} onClick={() => moveToPost(navigation.data?.previous_id, (navigation.data?.position ?? 1) - 1)} aria-label='上一条'><ChevronLeft className='size-4' /></Button><Button variant='outline' size='icon' disabled={!navigation.data?.next_id} onClick={() => moveToPost(navigation.data?.next_id, (navigation.data?.position ?? 0) + 1)} aria-label='下一条'><ChevronRight className='size-4' /></Button></div></div></SheetHeader>{detail.isLoading ? <div className='space-y-4 p-6'><Skeleton className='h-10 w-2/3' /><Skeleton className='h-72 w-full' /></div> : detail.data && <div className='space-y-6 p-6'><div className='grid gap-3 rounded-xl border bg-muted/20 p-4 sm:grid-cols-2'><Meta label='圈子' value={detail.data.circle_name} /><Meta label='作者' value={detail.data.author} /><Meta label='发布时间' value={formatDate(detail.data.published_at)} /><Meta label='平台帖子 ID' value={detail.data.platform_post_id} /></div><div><h3 className='mb-2 text-sm font-semibold'>正文快照</h3><div className='whitespace-pre-wrap rounded-xl border bg-background p-4 text-sm leading-7'>{detail.data.content || '正文为空'}</div></div><div><h3 className='mb-2 text-sm font-semibold'>一级评论（{detail.data.comments.length}）</h3><div className='space-y-2'>{detail.data.comments.length ? detail.data.comments.map((comment, index) => <div key={comment.platform_comment_id || index} className='rounded-xl border p-4'><div className='flex justify-between text-xs text-muted-foreground'><span>{comment.author || '匿名用户'}</span><span>{formatDate(comment.published_at)}</span></div><p className='mt-2 whitespace-pre-wrap text-sm'>{comment.content || '—'}</p></div>) : <div className='rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground'>没有已保存的一级评论。</div>}</div></div></div>}</SheetContent></Sheet>
