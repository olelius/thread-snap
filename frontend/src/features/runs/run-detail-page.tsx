...
import { api, ApiError, errorMessage, formatDate, platformName, queryString } from '@/lib/api'
...
function RunDetail({ kind }: { kind: RunDetailKind }) {
  ...
  const retry = useMutation({ mutationFn: () => api<Run>(`/runs/${runId}/retry`, { method: 'POST', headers: { 'Idempotency-Key': createUuid() } }), onSuccess: async (value) => { await client.invalidateQueries({ queryKey: ['runs'] }); toast.success('失败项已重新提交', { description: `新批次 ${value.number}` }) }, onError: (error) => toast.error('重新提取失败', { description: errorMessage(error) }) })
  ...
  // 与列表共用筛选和排序，不传分页参数；下载失败时保留当前表格上下文。
  const exportFiltered = useMutation({
    mutationFn: async () => {
      const filename = `${run.data?.number ?? runId}-筛选结果.xlsx`
      const response = await fetch(`/api/v1/runs/${runId}/posts/export${queryString(postQueryValues)}`)
      if (!response.ok) {
        throw new ApiError(response.status, await response.json().catch(() => ({ code: 'HTTP_ERROR', message: `请求失败（${response.status}）` })))
      }
    ...
    onSuccess: ({ blob, filename }) => {
      ...
      toast.success('筛选结果已导出', { description: '已包含全部分页，并保留当前筛选与排序。' })
    },
    onError: (error) => toast.error('导出筛选结果失败', { description: errorMessage(error) }),
  })
      ...
      <Sheet open={Boolean(search.post)} onOpenChange={(open) => { if (!open) closePostDetail() }}>
        <SheetContent className='w-full overflow-y-auto p-0 sm:max-w-[58vw]' onOpenAutoFocus={handleDetailOpenAutoFocus} onCloseAutoFocus={handleDetailCloseAutoFocus}>
            ...
            <div className='flex items-start justify-between gap-4 pr-8'>
              <div>
                <SheetTitle className='flex items-center gap-2'>{detail.data?.is_deleted && <StatusBadge value='unknown' label='删除' />}{detail.data?.title || '帖子快照详情'}</SheetTitle>
...
function ScreenshotPanel({ groups, loading, error, onRetry }: { groups?: ScreenshotGroup[]; loading: boolean; error: unknown; onRetry: () => void }) {
  ...
  // 只按本批次冻结的圈子名称筛选，不读取当前配置或猜测不同名称的车型映射。
  const vehicleName = (group: ScreenshotGroup) => group.circle_name?.trim() || `未命名圈子（${group.external_id}）`
  const vehicles = Array.from(new Set((groups ?? []).map(vehicleName)))
  const selectedVehicle = vehicles.includes(vehicle) ? vehicle : 'all'
  const visibleGroups = selectedVehicle === 'all' ? groups : groups?.filter((group) => vehicleName(group) === selectedVehicle)
  ...
  return <div className='h-full min-h-[360px] overflow-auto rounded-xl border border-border/70 bg-card/90 p-4' aria-live='polite'>
    <div className='mb-4 flex flex-wrap items-end justify-between gap-4'><div><h2 className='font-semibold'>圈子页面截图</h2><p className='mt-1 text-sm text-muted-foreground'>每个实际圈子来源独立保留原始全页证据；最终成果汇总当前有效条目，并一次性框出全部负面卡片。</p></div><div className='flex w-full flex-wrap items-center gap-2 sm:w-auto'><span className='text-sm text-muted-foreground'>车型</span><Select value={selectedVehicle} onValueChange={setVehicle} disabled={!vehicles.length}><SelectTrigger className='w-56 max-w-full' aria-label='筛选截图车型'><SelectValue /></SelectTrigger><SelectContent><SelectItem value='all'>全部车型</SelectItem>{vehicles.map((name) => <SelectItem key={name} value={name}>{name}</SelectItem>)}</SelectContent></Select><span className='text-xs text-muted-foreground'>显示 {visibleGroups?.length ?? 0} / {groups?.length ?? 0} 个来源</span></div></div>
    {!groups?.length ? <div className='rounded-xl border border-dashed p-12 text-center text-sm text-muted-foreground'>当前批次没有圈子来源截图。</div> : <div className='grid gap-3 xl:grid-cols-2'>{visibleGroups?.map((group, index) => {
...
function SentimentCell({ post }: { post: Post }) {
  if (post.is_deleted) return <StatusBadge value='unknown' label='已跳过（帖子已删除）' />
...
function PostDetailContent({ runId, post, onCorrect }: { runId: string; post: Post; onCorrect: () => void }) {
  ...
  return <div className='space-y-6 p-6'>
    ...
    <section className='space-y-3 rounded-xl border p-4'>
      ...
      <Button variant='link' className='h-auto px-0' asChild><a href={post.url} target='_blank' rel='noreferrer'>{post.is_deleted ? '打开原帖' : '媒体无法显示时打开原帖'}<ExternalLink className='size-3.5' /></a></Button>
    </section>
    <div><h3 className='mb-2 text-sm font-semibold'>正文快照</h3><div className='whitespace-pre-wrap rounded-xl border bg-background p-4 text-sm leading-7'>{post.content || (post.is_deleted ? '主楼已删除，未采集正文。' : '正文为空')}</div></div>
...
function ManualSentimentDialog({ open, onOpenChange, runId, post, onSaved }: { open: boolean; onOpenChange: (open: boolean) => void; runId: string; post?: Post; onSaved: () => Promise<void> }) {
  ...
  const save = useMutation({ mutationFn: (action: 'set_result' | 'restore_ai') => {
    if (!post || post.is_deleted) return Promise.reject(new Error('帖子已删除，已跳过舆情判定。'))
    return api(`/runs/${runId}/posts/${post.id}/sentiment/manual-revisions`, { method: 'POST', body: JSON.stringify({ action, result: action === 'set_result' ? result : undefined, primary_category: action === 'set_result' && result === 'negative' ? primary : undefined, secondary_categories: action === 'set_result' && result === 'negative' ? secondary : [], note: note.trim() || undefined }) })
  }, onSuccess: async () => { await onSaved(); onOpenChange(false); toast.success('舆情结论已更新') }, onError: (error) => toast.error(`${manualActionName}失败`, { description: errorMessage(error) }) })
