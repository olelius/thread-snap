...
function RunListPage({ kind }: { kind: RunListKind }) {
  ...
  const columns = useMemo(() => [
    ...
    column.display({ id: 'progress', header: '进度', cell: ({ row }) => { const run = row.original; const percent = run.planned_count ? Math.min(100, Math.round(((run.completed_count + run.failed_count) / run.planned_count) * 100)) : 0; const screenshot = run.screenshot_summary; const failuresRecovered = run.status === 'success' && run.completed_count >= run.planned_count; return <div className='w-40 space-y-1.5'><div className='flex justify-between text-xs'><span>{run.completed_count} / {run.planned_count}</span><span className='text-muted-foreground'>{percent}%</span></div><Progress value={percent} className='h-1.5' />{run.failed_count > 0 && <div className={`text-[11px] ${failuresRecovered ? 'text-muted-foreground' : 'text-red-600 dark:text-red-300'}`}>{failuresRecovered ? `跳过 ${run.failed_count} 个异常候选` : `${run.failed_count} 项失败`}</div>}{screenshot && screenshot.status !== 'not_applicable' && <div className='text-[11px] text-muted-foreground'>截图：{runScreenshotStatusNames[screenshot.status] ?? screenshot.status}{screenshot.group_count > 0 ? ` ${screenshot.ready_count}/${screenshot.group_count}` : ''}</div>}</div> } }),
    ...
    column.display({ id: 'action', header: () => <span className='sr-only'>操作</span>, cell: ({ row }) => <div className='flex justify-end gap-1'>{row.original.status === 'waiting_for_auth' && <Button variant='outline' size='sm' onClick={(event) => { event.stopPropagation(); setAuthRun(row.original) }}><KeyRound className='size-4' />去认证</Button>}<Button variant='ghost' size='sm' onClick={(event) => { event.stopPropagation(); navigate({ to: detailPath, params: { runId: row.original.id }, search: emptyDetailSearch }) }}>查看</Button></div> }),
