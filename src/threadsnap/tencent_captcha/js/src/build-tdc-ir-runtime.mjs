...
const interpreterPath = path.resolve(process.argv[3]);
const irPath = path.resolve(process.argv[4]);
const sampleName = process.argv[5] ?? "live";
const outputPath = path.resolve(process.argv[6] ?? path.join(root, "analysis", `${sampleName}-tdc-ir-runtime.js`));
...
const sample = ir.samples.find((item) => item.name === sampleName);
if (!sample) throw new Error(`sample ${sampleName} missing from handler IR`);
const handlerById = new Map(ir.handlers.map((item) => [item.id, item]));

const interpreterAst = parse(interpreterSource, { sourceType: "script" });
const protocolPreludeAssignments = [];
...
const discriminant = vmSwitch.path.node.discriminant;
...
const codeName = discriminant.object.name;
const pcName = discriminant.property.argument.name;
const innerFunction = vmSwitch.path.getFunctionParent();
if (!innerFunction) throw new Error("inner VM function not found");
const outerFunction = innerFunction.parentPath.getFunctionParent();
...
const outerParams = outerFunction.node.params.map((item) => item.name);
let recursiveName = null;
...
const transpile = (canonicalSource, bindings) => {
  const ast = parse(canonicalSource, { sourceType: "script", allowReturnOutsideFunction: true });
  traverse(ast, {
    Identifier(p) {
...
const opcodeEntries = sample.opcodes.map((opcode) => {
...
const config = { codeName, pcName, regsName, undefinedName, exceptionStackName, currentErrorName, recursiveName, outerParams, localNames, allBindings: [...allBindings].sort() };

const rawAst = parse(rawSource, { sourceType: "script" });
...
if (!payloadPath || payloadPath.node.value.length < 10000) throw new Error("VM payload not found");
...
const factoryInvocation = invocationStatement.node.expression.callee;
factoryInvocation.callee = t.callExpression(t.identifier("__tdcCreateVm"), [t.stringLiteral(payloadPath.node.value)]);
const invocation = generate(invocationStatement.node, { compact: true, comments: false }).code;
