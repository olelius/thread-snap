...
- `rpms/`：Python、Nginx、Xvfb 和 Chromium 系统共享库及其 RPM 依赖；
...
1. 校验输入包全部文件；
2. 下载并冻结 Python wheelhouse；
3. 在临时虚拟环境中执行纯离线安装与 `pip check`；
4. 下载锁定 Patchright 对应的 Linux Chromium；
5. 使用 `dnf download --resolve --alldeps` 收集系统 RPM 闭包；
6. 生成新的逐文件校验清单和压缩包整体校验值；
7. 输出 `*-centos-stream-10-x86_64-offline.tar.gz`。
...
- 从包内 RPM 安装 Python、Nginx、Xvfb 和共享库，禁用所有 DNF 仓库；
- 建立 `threadsnap` 系统账号；
- 在新 release 中建立虚拟环境并从本地 wheelhouse 安装；
- 从包内复制 Chromium；
- 首次生成 Fernet 密钥，升级时保留原密钥；
- 安装并启动 Xvfb、ThreadSnap 单进程和 Nginx；
- 设置 SELinux 静态文件与回环代理策略；
- 执行健康检查，失败时恢复上一程序版本。

防火墙只向受控内网或 VPN 网段开放 Nginx 端口。FastAPI `8000`、CDP 和 Xvfb 均保持本机访问。

## 6. 验证

```bash
sudo bash /opt/threadsnap/current/deploy/verify.sh
sudo systemctl status threadsnap threadsnap-xvfb nginx --no-pager
sudo journalctl -u threadsnap -n 200 --no-pager
```

脚本检查 systemd、Nginx、SPA、API、`/internal/v1` 屏蔽、端口绑定、Fernet 配置和 Xvfb 有头 Chromium启动。此后仍需真实执行：
