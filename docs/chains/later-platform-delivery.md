...
- 当前阶段：汽车之家已完成一个真实社区的两个列表顺序、完整候选发现和休眠适配器实现；匿名详情预检在形成295条有效结果后进入平台访问验证，因此尚未冻结或运行正式500条，平台注册继续保持 `not_integrated`。易车仍由独立工作线盘点。
...
| 2026-08-27 | 汽车之家先合入休眠适配器，不提前打开平台注册 | 真实来源与候选池已证明，但匿名预检遇到访问验证，500/500和实际视频URL合同仍未关闭 |

## 未决项

- [ ] 继续盘点汽车之家、易车当前可用社区来源及两个列表顺序；汽车之家已确认 `iCAR V27论坛` 的最新回复与最新发布关系并形成Git外来源清单，完整业务范围尚未宣称盘点完成。
- [ ] 为每个平台生成、现时复核并冻结500条帖子详情URL及分层功能样本。
- [ ] 完成两个平台适配器并分别通过 `500 / 500` 正式验收；汽车之家休眠适配器和公共行为测试已落地，但正式清单、实际视频媒体合同及可持续访问模式仍待关闭。
