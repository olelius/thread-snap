...
| 2026-09-01 | 易车腾讯 WAF 逆向调研完成、生产保持ADR 0058 | 固定历史`js-reverse-mcp@1.2.1`并通过静态门；两个零重叠80 URL轮次均在前59个正文后由第60个请求命中验证码并止损，冻结AppId、SDK/iframe/TDC、双图和WAF回调链；本地识别在两个样本取得唯一峰，固定轨迹单次通过腾讯verify并触发WAF POST。原风险会话到详情正文门禁未关闭，故只保留研究产物，不加入生产采集器 |
...
- [x] 易车WAF隔离逆向已冻结SDK/iframe/脚本/双图/网络和回调链并完成本地识别及单变量腾讯verify；生产决策为只保留研究产物，原风险会话正文门禁保持开放且不阻塞ADR 0058现状。
