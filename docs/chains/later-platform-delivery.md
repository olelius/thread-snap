...
| 2026-09-01 | 易车并发1不能作为免控阈值 | 固定1000个不同详情URL的独立批次在59/1000进入腾讯验证码，失败0并保留剩余941条；降低请求突发不等于平台不再控制，恢复必须继续沿用原批次和剩余清单 |
...
- [ ] 两平台本地适配器、媒体合同和公共/组合测试已落地，均已发布为 `available` 且默认停用；汽车之家继续关闭真实登录Session、正式清单和认证模式 `500 / 500`，易车当前固定1000条诊断批次在59/1000等待腾讯验证码，完成原批次恢复及后续可持续访问模式与真实 `500 / 500` 后，再声明对应平台正式生产接入验收完成。
