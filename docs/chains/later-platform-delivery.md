...
- 当前阶段：汽车之家与易车均已完成本地适配器和公共业务闭环并发布为 `available`，默认停用、并发上限1。汽车之家 `autohome-club-v3` 已使用官方账号登录、加密Session和直连HTTP读取可信点赞；易车 `yiche-community-v2` 已改用相同账号登录状态机，以账号Cookie和官方用户身份接口作为门禁，并以 `pc-v311` 直连HTTP取得列表、详情及评论。三平台后台帖子采集都不回退Chromium；人工登录和明确页面证据仍各自使用受控浏览器。两平台都尚未冻结或运行正式500条，正式生产验收仍未关闭，易车未观察评论翻页组合与页面证据继续作为明确缺口。
