...
- 架构决策：`docs/adr/0043-use-project-discovered-500-sample-gate-for-later-platforms.md`、`docs/adr/0044-separate-local-adapter-publication-from-formal-sample-acceptance.md`。
- 当前阶段：汽车之家与易车均已完成本地适配器和公共业务闭环并发布为 `available`，默认停用、并发上限1。汽车之家匿名预检形成295条有效结果后进入正式环境访问验证，易车已完成两个列表顺序、详情、一级评论、图片、视频和控制页取证；两平台都尚未冻结或运行正式500条，正式生产验收仍未关闭，易车未观察评论翻页组合与页面证据继续作为明确缺口。
...
- [ ] 两平台本地适配器、媒体合同和公共/组合测试已落地，均已发布为 `available` 且默认停用；继续分别关闭正式清单、可持续访问模式与真实 `500 / 500`，通过后再声明对应平台正式生产接入验收完成。
