...
| 2026-09-01 | 易车腾讯 WAF 逆向调研完成、生产保持ADR 0058 | 固定历史`js-reverse-mcp@1.2.1`并通过静态门；两个零重叠80 URL轮次均在前59个正文后由第60个请求命中验证码并止损，冻结AppId、SDK/iframe/TDC、双图和WAF回调链；本地识别在两个样本取得唯一峰，固定轨迹单次通过腾讯verify并触发WAF POST。后续完全协议化研究已用自研TDC IR在两个新challenge取得`errorCode="0"`，并关闭同会话WAF POST与原正文门禁；生产仍不接入该高漂移链 |
...
- [x] 易车WAF逆向结果已按ADR 0060封装为平台无关腾讯组件并由易车v8调用；全新腾讯challenge取得95-opcode/74-handler业务成功，当前易车Session在第60个详情命中WAF后自动回调并恢复正文，失败路径仍由ADR 0058承接。
