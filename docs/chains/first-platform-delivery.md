...
- [ ] Linux 独立联通门 —— 环境、浏览器、网络及 A/B 各自短信会话均已通过，Candidate B 已在同轮3/3成功；Candidate A 的普通帖子完整 `load` 等待已定位并修正，等待目标 Linux 覆盖 DOM 就绪热修后复跑。联通汇总仍为 `ready_for_2000=false`，本阶段门尚未关闭。
