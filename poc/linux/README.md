...
该入口只用于当前 PoC 测试，不构成第一版功能实现。第一版不保存平台账号密码：浏览器和 Session 位于服务器，客户从受控内网客户机的普通浏览器经基础测试界面的临时嵌入式入口操作服务器浏览器中的官方页面；出现登录表单、扫码、短信、验证码或滑块时，原批次持久化暂停，认证和门禁通过后自动续跑，不实现自动接码或无限重登。公网接入仅为后续可选增强。
