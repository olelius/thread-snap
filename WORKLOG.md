...
**状态**：🟡 会话交接修复与本机回归已完成；等待生成 v0.2.13 热修包并在目标 Linux 复跑联通门
...
**下一步**：完成全量测试、静态检查、凭证扫描和文档一致性检查；生成免重装 v0.2.13 热修包。目标 Linux 覆盖后直接复跑 `test-connectivity.sh`，先确认 `prepare.log` 中 A/B 的 `session_state_copied=true`，再以汇总 `ready_for_2000` 决定是否进入 2000 条。
