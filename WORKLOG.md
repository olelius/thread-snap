...
**状态**：🔄 目标 CentOS 首轮2000条已完成复核：可取得详情的1645条字段完整率为100%，355条来源已丢失；正在把确认后的标题、纯媒体和状态规则写入提取器并生成免重装补丁。
...
**下一步**：完成提取器测试与0.2.20免重装补丁验证；随后事前形成当前可访问的固定2000条新样本，在目标Linux执行连续轮次。单条P50/P95仍包含队列等待，只作为排队耗时，不解释为HTTP网络延迟。
