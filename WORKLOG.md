...
- 2026-08-20：千问业务空间在真实队列运行中返回 `HTTP 403（insufficient_quota）`；剩余 13 条已安全暂停，须补充该业务空间配额或更换并重新验证可用连接后才能继续。
...
## 2026-08-20 — JSON Object 软约束兼容与真实队列恢复
**总目标**：兼容千问 JSON Object 模式实际返回的枚举/字段关系偏差，在不重复请求已有原始响应的前提下修复失败记录，并继续处理暂停队列。
**状态**：⚠️ 代码修复、全量验证、4 条本地恢复和真实队列重启均已完成；队列推进到 17 条完成后被提供方 `insufficient_quota` 阻断，剩余 13 条保持可恢复暂停。
**干到哪里了**：
- [x] 真实批次稳定现场为 9 条完成、4 条结构失败、17 条暂停；4 条失败均已保存可解析原始响应，偏差分别包含 `present` 顶层状态、`relevant` 逐图片状态和非负面结果附带正向分类，不是 HTTP 或模型无返回。
- [x] 提示词升级为 `v3` 并完整列出状态与负面分类约束；新增同一生产解析入口，只在输入存在、处理计数完整、依据非空且媒体身份最终校验通过时收敛 `present` / `relevant` / `irrelevant`，不改变模型相关性、情感、总结和依据。
- [x] 模型明确给出不相关或非负面时只清空规范化负面分类；负面合同外主要类型规范为 `other`、合同外次要类型不进入稳定字段，提供方原始响应原样保留并标记本地恢复。
- [x] 新增本地用例覆盖真实 `present`、`relevant`、被截断/拼接 URL 哈希、正向分类、合同外负面分类、HTTP 200 流内错误和 HTTP 错误代码脱敏提取；78 项后端测试、Ruff、`compileall`、`pip check`、前端 TypeScript 检查、生产构建和 `git diff --check` 通过。
- [x] 停止旧后端后备份数据库到 `artifacts/runtime/sentiment-json-flex-20260820-170115/threadsnap-before-recovery.db`，源与备份 SHA-256 均为 `EDCCEEC9D854F363AC109541BCA6C4F74F4A130799CF734E558AF9C6E20E6198`；4 条已有原始响应全部由生产解析入口本地恢复为 `analysis_completed/non_negative`，保持实际生成它们的提示词版本 `v2`，没有重复请求。
- [x] 修复版后端重启并恢复两个消费者后，真实批次由 13 条完成推进到 17 条完成；两条 HTTP 200 空流已补齐流内错误分类与请求审计保存，但受控重试随即收到 HTTP 403。一次最小连接验证取得明确代码 `insufficient_quota`，配置已自动关闭并标为 invalid，批次最终为 17 条完成、13 条暂停、0 条运行/排队/失败。
- [x] 后端 `/health`、Vite `/health` 代理均为 `ok`，前端 HTTP 200；修复版后端和既有前端继续运行。
**下一步**：补充当前业务空间配额，或保存并测试另一组具有该模型权限和额度的连接；连接测试通过后在配置页重新开启，13 条暂停任务会恢复排队并在领取时绑定提示词 `v3`。
**边界**：4 条已有原始响应不重复付费请求；本轮新增完成 4 条，两条空流是否计费因旧客户端未保存 usage 暂时无法确认；403 与失败连接测试没有形成模型正文。原始帖子快照和模型原始响应不改写。
