...
## 2026-08-27 — 汽车之家真实来源预检与休眠适配器
**总目标**：在独立工作树中确认汽车之家真实论坛来源、两个列表顺序和字段合同，实现统一采集器适配与生产门禁，并以正式500/500为唯一启用条件。
**状态**：🚧 来源、候选池、匿名预检、休眠适配器和公共行为回归已完成；现时平台访问验证、实际视频URL及正式500/500仍使接入门保持开放。
**干到哪里了**：
...
- [x] 新增共享采集器协议、平台注册表和 `autohome-club-v1` 适配器；后端、Worker、认证路由及前端平台能力均从注册表读取。汽车之家保持 `not_integrated`、默认停用、并发硬限1，页面证据、实际视频URL和正式认证能力关闭。
- [x] 适配器交叉校验列表、URL、详情帖子ID和论坛ID，只把正文或实际图片作为内容证明；前10条SSR一级评论及一页终止证明、显式删除三态、控制响应首条止损、重复/无界分页停止均有专项测试。单条和批量来源验证同时过滤未接入平台，遗留队列在Worker侧再次拒绝。
- [x] 独立审查发现未接生产Provider且可改变验收分母的通用验收脚手架，已从整合结果撤回；正式验收继续固定500，不留下可缩小分母的入口。
- [x] 项目虚拟环境后端完整发现测试155/155、PoC共享测试64/64、Ruff、compileall、pip check和 `git diff --check` 通过；前端TypeScript检查与生产构建通过，Vite共转换2468个模块。
**下一步**：在新的冷却访问窗口或经真实样本证明的正式汽车之家Session模式下，从零复核至少500个候选；先补齐实际视频URL合同，再冻结500条并完整运行500/500，成功后才把平台注册切换为可用。
**边界**：本轮295条有效预检不与后续结果拼接，不把HTTP 200、标题或未解析视频ID当有效内容；未修改服务器、未启用汽车之家、未生成正式500清单、未执行口碑映射或新增口碑指标。
**关联**：`docs/chains/later-platform-delivery.md`、`docs/research/later-platform-onboarding-plan.md`、`src/threadsnap/collectors/autohome.py`、Git外 `artifacts/runtime/autohome-onboarding/`
