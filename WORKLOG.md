...
**状态**：🟡 v0.2.4 已确认目标 Linux 强制二次短信验证；正在修复候选 A 登录入口资源等待并准备复跑包
...
**下一步**：生成候选 A 资源等待修复与强制短信标记的免重装热修包，在目标服务器覆盖后复跑联通脚本。若仍显示二次短信验证，则先用该账号在目标服务器访问条件下完成一次手机验证并建立持久会话，再复跑联通门；只有复核为 `ready_for_2000=true` 后才运行首轮 2000 条。
