...
**总目标**：把第一版正式登录方式从“数据库明文保存平台账号密码”调整为“官方页面人工认证、加密 Session、有界自动刷新”，同时保持当前懂车帝单平台基础闭环范围。
...
- [x] 人工认证必须在提取功能服务控制的浏览器中打开平台官方页面；普通外链在用户本机形成的登录状态不等于服务器采集 Session。
- [x] 身份异常先暂停新请求并保留触发 URL，每个失败事件只对既有加密 Profile 自动刷新一次；官方页面仍登录且少量门禁通过时从触发 URL 继续。
- [x] 出现登录表单、扫码、短信、验证码、滑块或刷新后门禁失败时，本批次以 `authentication_required` 结束；用户更新 Session 后人工重试，不无限重登或长期挂起批次。
...
**下一步**：正式功能开发时先落地平台 Session 状态模型、加密存储边界、受控官方登录入口和一次自动刷新状态机；远程浏览器交互组件随最终前后端与部署技术栈一并选择。

**边界**：本次只确定第一版平台认证与 Session 生命周期，不新增多账号、账号轮换、自动接码、验证码识别或长期浏览器托管，也不扩展当前第一版业务范围。
