...
**状态/下一步**：开发与必要验证完成。3项稳定窗口用例、1项实际适配器→隔离SQLite数据/证据分离组合、3项既有身份门禁共7项2.295秒通过；仅补2项受影响懂车帝截图门禁（7.662秒），请求后截图复核变动只重验该1项（1.097秒）。Ruff、compile与diff-check通过；未跑全库/多轮审查/整批重采。下一步仅完成本提交PR合并与分支清理。
**真实页面证据/回退**：长安CS75PLUS单次真实页面2.844秒成功、评分4.50；截图解码/hash一致、已目视车型/评分/数量区域，源业务库hash未变。artifacts/runtime/layout-settle/{check.log,dongchedi-check.log,capture-after-request.log,live/result.json,live/capture/}存证据；截图1px容差不应用于核心文字，URL-only不设截图几何门。可靠指标与截图错误分别保存，映射验证截图失败仍不提升verified。无迁移，可撤销本提交；未部署远端。

---

## 2026-09-17 — 口碑巡检批次统一失败重试
**目标/基线**：用户确认覆盖所有可能重新采集恢复的失败，不限定首页或超时；从origin/main@48fad83建立fix/reputation-batch-retry-waves与独立工作树，保留原混合修改。
**口径/状态**：开发与定向验收完成。正式巡检首轮后按跨平台整轮屏障最多追加3轮，单次采集从90秒缩短至30秒（取消清理另有3秒上限），成功项不重采，并发2/总45分钟不变；认证/验证码/限流和确定性无效配置独立收口。映射验证仍为独立合同。
**验证/下一步**：6项定向测试最终4.924秒通过，2项既有线性进度/二次恢复用例在整合检查通过；Ruff与diff-check通过。隔离SQLite真实ReputationCoordinator→execute_run→HTTP API→汇报链覆盖3平台8目标，实际尝试1/2/3/4/2/4/1/1，6成功2明确终态，正常成功项只访问1次；未知错误、解析错误、缺截图、超时、认证、中断恢复、预算耗尽均有覆盖。证据artifacts/runtime/reputation-retry/{final-affected.log,integrated-check.log,combined-result.json}，三平台真实适配器导航挂起的截止测试不访问外站。接下来仅完成本提交PR合并与分支清理，不跑全库或重复回归。
**边界/回退**：不自动部署、不改远端数据与历史批次；无依赖或数据库迁移，回退本次代码即可。详细过程和子任务回执存于artifacts/runtime/reputation-retry/。

---

## 2026-09-15 — 跨批次帖子全局复用
**目标/范围**：从最新 `origin/main@a6fde171` 创建独立分支，按 `(platform_code, platform_post_id)` 复用三平台历史完整快照；不跨平台裸 ID 去重，新批次仍保存独立快照。
...
## 2026-09-15 — 修复旧合同映射无法重新验证
**总目标**：让已验证但合同版本过期的车型平台映射进入重新验证队列，避免页面显示已验证却没有可点击的验证目标。
**状态**：✅ 修复已合并并部署。旧合同映射现在进入重新验证目标；前端类型检查、生产构建和 6 项前端测试通过。线上当前为 `0.1.0-a92843954577`，previous 为 `0.1.0-852dd19b92e5`，三项服务、双健康接口和易车旧合同草稿状态已核验。
**实现**：前端将 `validation_contract_version` 与平台当前合同比较，保留易车原生合同兼容；按钮会对旧 `yiche-reputation-mapping-v1` 的完整映射提交 `vehicle_ids`，后端按当前合同生成新的验证记录。新增旧合同回归用例。
**边界**：不手工改写验证状态、哈希或历史尝试；24 项缺少易车映射仍需补齐，发布范围继续只读取当前有效组合。
**下一步**：刷新线上“车型与映射”页面；易车当前 27 项旧合同映射会进入重新验证队列，24 项缺失映射继续单独提示。验证完成后再发布新范围版本。
...
**实现**：发布预览/事务共用筛选逻辑，只把当前验证合同、映射哈希均有效的组合写入不可变 `reputation_scope_versions`；调度 `target_keys` 与真实验收按实际组合计数；前端确认框展示本次发布车型、已验证映射和跳过项。未新增数据库表，草稿和验证历史保持完整。
**边界**：未验证、失败或缺失组合不进入本次版本；历史批次与草稿不改写；后续补齐后再次发布生成新范围版本，新组合按新映射基线处理。
**下一步**：刷新线上“车型与映射”页面；发布确认框将只列出当前已验证的车型平台组合，并显示暂不发布数量。未验证组合留在草稿，补齐后再发布新版本。

---

## 2026-09-15 — 修复口碑映射验证批量目标选择
**总目标**：修复“验证待验证（N）”未传车型清单、遇到缺映射导致整批 400 的问题，并按最小更新包发布到生产服务器。
**状态**：✅ 修复已合并并部署。后端受影响回归 2 项、前端类型检查、生产构建和 5 项前端测试通过；线上已切换到 `0.1.0-4cfa424163fd`，当前/previous、三项服务、双健康接口和新前端资源均已核验。
**实现**：验证按钮仅提交当前平台启用且映射字段完整、状态非 `verified` 的车型 ID；缺少映射的车型单独显示并跳过，不再让后端全量预检提前中止。已验证项不重复访问。新增 `frontend/src/features/reputation/reputation-validation.ts` 及 `frontend/tests/reputation-validation.test.mjs`。
**边界**：发布范围仍要求全部启用车型 × 平台通过；本修复只改变验证入口目标选择，不放宽发布门禁，不修改历史验证记录或批次。
**下一步**：用户刷新线上页面后点击汽车之家验证；按钮将提交映射完整的待验证车型ID，缺失映射单独提示并跳过。发布门禁与历史批次规则保持不变。
...
**缓存/历史**：新增字段值与截图身份只读冻结供渲染和指纹共用；f3b6c9d2a804仅为export_records追加输入指纹并扩充唯一键，旧记录为空指纹，原22字段模板沿用旧缓存，新增字段变化生成独立导出，历史文件保留。删除帖不输出过往AI/人工结果，不触发采集、模型或截图重建。
**证据**：旧22字段注册元数据SHA256仍为e80bb37bd362589f087a9ccfef00c46fbb52a00eee050d3c20a99eb1b3dd1dc4；三平台153标签唯一。首轮旧模板相关回归通过，4项新增测试的表头误用保留前缀修正后通过；之后只重验图片读取/排版受影响项，最终窄列与冲突2项2.356秒通过。前端build含tsc、定向Ruff、diff-check通过。业务库副本迁移保留3条原导出完整内容与41表记录数，foreign_key_check为空；三平台各30条真实结果导出50个文本字段及独立PNG，3/3嵌入原图hash不变。最终浏览器从模板入口真实下载的SHA256=974370ae61b36734e7104c1ac8603ff6ce942bea5e92cfa243c220c2b6d17d8b，三平台字段数/来源标识、截图搜索和空搜索通过，无页面脚本错误；Excel16原生打开确认窄列、120pt单行、独立图片，已目视最终图。
**入口/下一步**：artifacts/runtime/template-fields/{tests.log,affected-tests.log,compact-layout-tests.log,build.log,real-exports.json,browser-report.json,native-excel.json,template-fields-ui.png,template-native-excel.png}。测试副本已清除登录会话引用，未改正式会话；完整原尺寸跨行方案已按用户参考撤回，早期预览不是最终格式。完成本条提交/PR收尾后，仅在本地无运行任务时备份现库、更新同一后端并核对51字段生效；生产服务器发布另行处理，不重复全量测试或采集。
**回退**：回退本功能代码；迁移尚无多输入导出版本时可降级，存在多版本则保留新结构，禁止删历史导出以满足旧唯一键。停机窗口数据库备份仅供整体一致回退，不覆盖后续业务写入。
...
## 2026-09-11 — 根治颜色误吸附，绑定截图与DOM几何
**状态**：独立分支codex/fix-evidence-bound-frames基于main@87c3107；核心实现与定向验证完成，待最小包部署及远端7个已确认受影响成果版本化重生。
**根因与方案**：删除颜色差/纹理边界搜索；原始PNG/清单哈希、帖子ID/URL/顺序、数据库矩形及页内不重叠校验后直接按DOM画框。新v2证书核对截图前后几何与PNG尺寸；截图浏览器从启动原生保持无滚动条占位，保留1440×900初始视口，不注CSS、不拉高视口移动浮层。客户端/文档/PNG同宽，避免CDP截图内部重排漏过前后相等检查。几何暂态按原任务2秒起、60秒上限退避自动续作，复用原页和已完成帖子，不直接终态报错，也不发布错图。
**历史边界**：原图、原清单、AI结论与旧成果不覆盖；v7-evidence-bound-dom-frames通过输入hash生成新版本。旧DCD v4/v7已有无同宽证明的累计漂移，不盲目套用旧DOM；保留旧成果。当前可确认汽车之家同类错位7组；本轮不修此前字体下沿裁剪、不重新访问平台或重判AI。
**证据**：捕获helper7项、截图与版本生命周期13项、Worker几何续作2项定向场景最终通过；新增合同导致的旧fixture预期/metadata缺项仅重验受影响项。离线经典滚动条样本35卡片最大累计偏104px，原生同宽方案Windows及目标Linux headed Wayland都35/35逐像素舍入对齐，固定浮层位置不变。T9现有50帖/4负面用新算法离线重生，喷粉框由[108,553,1013,89]回到[112,647,880,73]，38720个变化像素仅位于4条红框线，原图/原清单/旧图hash不变；顶部/底部截图已目视。代码静态检查通过，未运行全库或真实AI/采集。
**产物/下一步**：捕获/恢复证据H:/ThreadSnap-capture-bound-geometry/artifacts/runtime/capture-geometry；目标Linux/旧图清单/恢复计划位于/var/tmp/threadsnap-frame-fix-20260911；T9预览H:/ThreadSnap-evidence-frame-preview/artifacts/t9-v2/receipt.json。完成提交合并后复用上次最小包/SELinux恢复标签流程更新远端，随后只为有可靠证据且受影响的组生成新版并核对当前版本，不重画来源不足的历史。
...
**部署/数据**：复用既有XFS reflink、双空闲门、Nginx先停/后端先起、维护期关闭后台与外部写入流程；数据库c3f7a1d9e402→e7a4c8d2b601，17表原有列/行哈希不变，新账户字段按默认账户1初始化。未同步本机预览库/测试账户或批次，未调用平台采集/模型/余额接口。备份/var/lib/threadsnap/backups/minimal-release-upgrade/20260908-main-659ddb8，SHA256 216b0f6398c4bdaa16860763ac522c34eba67e5c56b6f93294ee2309296582fe。
...
**边界**：不增加数据库表或迁移，不做备份，不实际调用删除接口；由用户自行删除目标账户。仅更新本地服务，旧易车分支与原目录不动；PR https://github.com/olelius/thread-snap/pull/289，Git记录见artifacts/runtime/ai-account-delete-20260908/pr-result.json。
...
**边界**：无数据库结构变更、备份或采集；仅更新本地服务加载规则，未提交或部署远端。

---

## 2026-09-08 — AI账户切换保持滚动位置
**状态**：📝 已修改前端代码，待用户查看；按原要求未测试、构建、审查或验收。
**改动**：移除按accountId强制重挂载；以已加载账户配置初始化并原位切换草稿，不用短加载卡替换整个编辑器；切换完成后恢复当前标签scrollTop，下拉关闭以preventScroll恢复焦点。保留未保存保护，账户切换清空密钥草稿且禁止旧草稿提交到新账户。
**边界**：仅配置页及owner/账本更新；未改后端、数据库或服务，不做备份/重启/提交。由现有Vite加载修改，用户刷新后查看。

---

## 2026-09-08 — AI账户控件收纳至模型连接
**状态**：📝 已按用户截图修改前端代码，待用户刷新查看；未运行测试、构建、审查或验收。
**改动**：移除顶部跨栏账户选择卡及独立名称/余额卡；在“模型连接”内放240px账户切换下拉与新增按钮，名称和余额紧凑并排，余额刷新用图标，明细与查询时间保留提示；账户查询、切换与dirty保护逻辑沿用。
**边界**：仅修改配置页排版及owner说明，未操作后端或数据库、未备份、未重启服务、未提交发布；前端由现有Vite加载修改。

---

## 2026-09-08 — 命名AI多账户与提取入口选择
**总目标**：新增可命名AI账户，规则和手动提取按名称选择，展示账户余额。
**状态**：📝 开发改动已完成，待用户自行查看；按用户明确要求未运行测试、构建、lint、审查或结果验收，不宣称通过。
**开发内容**：复用sentiment_configs新增name并保留账户1；账户独立模型/加密连接/判定对象/并发与连接状态。新增账户不复制密钥，旧GET/PUT/config/test可按account_id选择；新增accounts列表/创建及余额接口。DeepSeek余额固定GET https://api.deepseek.com/user/balance，Bearer使用所选Key，币种和金额保留原字符串；其它服务、缺Key或查询失败给出明确状态，不补零。
**任务路由**：规则版本保存ai_account_id，手动圈子及URL模式均传递；批次/来源任务冻结ID与名称，分析记录保存实际账户。Worker按账户领取、按账户缓存/暂停/恢复/429冷却，账户并发合计受进程64上限约束，本地模型全进程单路。相同节点同来源的AI启用规则选不同账户时在保存/调度明确阻止，不静默换号或重复计费。
**前端**：AI舆情账户选择、新增命名/重命名、独立dirty保护与余额卡；规则和手动下拉按名称选择，账户缺失不回退其他账户。沿用现有组件，无新增管理页面；前端3个文件由独立工作树单写手实现并复制整合，回执artifacts/runtime/ai-accounts/ui-receipt.md；主线程补充SSE账户列表失效通知。
**本地启动/未验证**：新增迁移e7a4c8d2b601，以默认账户1衔接旧规则与分析记录。仅备份现有预览库到artifacts/runtime/ai-accounts/before-accounts-20260908-163934.db，并发出更新后本地API启动请求（启动进程57228）；未读取启动日志、未查health、未调用账户/模型/余额接口或浏览器核验，迁移与运行效果均未验收。配置入口http://127.0.0.1:5173/config。原H:/ThreadSnap业务库未操作，远端未部署。
**下一步/边界**：当前feat/ai-accounts保留未提交开发改动，由用户查看后反馈；没有质量门证据，不自动提交推送合并。多个账户及新历史关联存在时迁移降级会主动停止，回退前需保留数据库备份。临时前端工作树H:/ThreadSnap-ai-accounts-ui保留，未清理用户目录。

---

## 2026-09-08 — 去掉模板一标题“较上日”括注
**状态**：✅ 仅将“懂车帝车友圈露出数(较上日)”调整为“懂车帝车友圈露出数”，下方增减说明和模板二不变；本地API已刷新。
**证据/边界**：受影响单测1/1与一次真实页面文字检查通过，721A正文仅指定字样变化；artifacts/runtime/report-templates-20260908/label-edit/check.json及updated.png。未重新采集、未改数据或远端；下一步供用户刷新查看。

---

## 2026-09-08 — 详情页选择两种重点车型汇报模板
**总目标**：在现有汇报结果页选择用户两模板，只展示重点车型，复制/TXT一致，并接入汽车之家论坛顶部帖子总数。
**状态**：✅ 本地实现与目标页面检查完成；新预览批次RP-A-20260908-080311-721A（01a0800a-e0c0-7baa-85c9-941b6cf9721a）可查看两模板；未部署远端。
**实现/边界**：模板一按车型列七项，口碑帖使用评价篇数；模板二只列口碑分、排名或懂车帝差评率有前日可比变化的重点车型。选择在本机记忆，复制与带template参数TXT同源；只使用原批次冻结结果，不混入晚到补跑，不覆盖原report_text或无参数TXT。按后续确认直接在详情页选择模板，不扩建项目组管理。
**论坛来源**：用户8433截图顶部帖子7083，现有列表API同ID的total实际2607；正常HTTP页面经DOM解析在#js-bbs-info的“帖子”strong直接取得7083。适配器追加一次同ID论坛HTML请求，核对URL、bbs类型/ID及相关车系，复用平台隔离circle_content_count，单独保留来源与SHA；“万”原文标rounded，只按同精度显示值比较，不充当精确数。评分PNG不证明论坛计数。
**本地数据**：仅在已有预览副本补采汽车之家，初轮12/27成功，15项页面“万”格式识别后仅补失败15项并全部成功；本轮成功27/27，其它两平台复用当天已有验证。721A冻结81项/54PNG，汇报只显示14款重点。原始数据和5824历史保持，新旧来源时间明确，不宣称三平台重新采集或新正式调度事件。
**必要检查**：新增6项定向测试通过；新增“万”格式后仅重验受影响2项通过。Ruff/compileall、触感前端构建和diff检查通过，未跑全量回归或追加独立审查。一次目标浏览器检查两模板/复制/TXT，14重点/27论坛值符合，JS错误0、业务写请求0；721A无基线不编造变化，旧真实日检FCE5用于展示模板二实际变化。剪贴板检查等待异步写入并规范化Windows换行，TXT保持API UTF-8字节一致。旧5824API除新增模板字段外与原保存响应相同。
**证据入口**：artifacts/runtime/report-templates-20260908/的targeted-tests.log、page-check.json、vehicle_detail.png、daily_changes.png、daily-changes-existing-run.png、template-run.json、autohome-validation.json及autohome-count-retry.json；forum-header8433-decoded.json与forum8433-source.json区分顶部与列表计数。
**下一步**：用户查看本地http://127.0.0.1:5173/reputation/runs/01a0800a-e0c0-7baa-85c9-941b6cf9721a?view=report后按反馈继续。前端5173与后端8000保持运行，后台调度关闭；当前分支feat/reputation-report-templates，回退源码不需迁移或覆盖数据库，原始存档保留。

---

## 2026-09-08 — 本地新增独立圈内内容数
**总目标**：按用户确认新增懂车帝圈内内容数，复用车型映射ID和圈子访问规则，仅更新本地供查看；保留口碑量/评价篇数及历史。
**状态**：✅ 本地全链实现与真实验收完成，5173前端/8000隔离预览API保持运行；本地分支feat/reputation-circle-content供用户查看，未SSH或部署远端。
**实现**：ADR0071；新键circle_content_count只在懂车帝出现，使用现有圈子URL规范与车型ID读取动态最新发布页，单次GET、不读首帖或翻页；校验URL/两个SSR身份字段/展示名，冻结SSR与可见计数、来源URL、时间和响应SHA。映射验证、正式采集、真实验收、比较、网页、TXT/XLSX接通；有效0保留、缺值不补零、变化中性、历史未采集。不增加字段配置、表迁移或PNG分母。
**真实验收**：只读原本机库备份到runtime预览副本并关闭后台服务，使用原本机v3范围不改发布；三平台本轮新验证各27/27成功，懂车帝圈数与差评率各27/27有值，生成RP-A-20260908-070817-5824（01a07fd8-9eb7-7bf1-a8b5-e8b7bc735824），81/81成功、54/54所需PNG，耗时07:05:13至07:08:18 UTC。风云A9本次圈数1909；早期两样本与用户截图仅是各自采集时点，不作为当前常量。
**验证**：最终相关自动化103/103（preview/final-tests.log）、后端/dashboard110/110、独立来源26/26，TS检查与默认/触感双构建通过；主线程浏览器17/17、JS错误0/业务写请求0，桌面浅/深色、两页面新列和其它平台无占位、旧批次历史未采集、浏览器独立中性变化夹具已目视。390px仅确认文档无横溢出，已有紧凑屏幕表格高度问题未作为本轮桌面验收通过。真实导出432/432指标与API逐项一致，21列/备注U、27张组合预览锚点正确，TXT与API UTF-8字节一致。独立只读审计10张口碑表既有1341行/20748字段不变、范围3/3不变，新54PNG实际解码及SHA通过。
**发现/修复**：真实导出暴露基线已有易车正常无图误写“证据缺失”；已按冻结evidence_required修正，仅真正要求截图的缺图报警，缺整项另标。14/14专项及真实只读派生导出验证通过，432指标/27锚点/54PNG不变；5824原冻结下载保留，修正文件单独保存为export-remarks-fix/derived-reputation-5824-remarks-fixed.xlsx，不冒充已替换原文件。
**证据入口**：artifacts/runtime/circle-content-local-20260908/，各子任务短回执及源码/构建SHA同目录；preview/local-execution.json、browser-verification.json、export-verification.json和review/preview-history-verification.json。预览http://127.0.0.1:5173/reputation/runs/01a07fd8-9eb7-7bf1-a8b5-e8b7bc735824。
**边界/下一步**：供用户本地查看后按反馈继续；本轮不触发正式10:00调度、不恢复停用车型、不修改原业务DB或远端，原H:/ThreadSnap的20项研发改动保留。飞书正文未取得，需求依据为截图与用户确认。回退只需恢复本功能源码并重启原本地服务；预览副本/冻结产物保留，不用副本覆盖原库。
...
**交付/回退**：已验证5173接收整页刷新后加载三个增强卡片与品牌区，真实累计值80/0/14；`artifacts/runtime/react-bits-home/delivery-verification.json`、`home-final.png` 和 `react-bits-preview.webm` 为预览证据。回退恢复本次前端调用点与样式并重建，不涉及数据库；完整许可随资源保留。
**下一步/边界**：保持首页预览供用户交互查看；后续只按新增反馈调整。原工作树20项未提交修改、数据库和运行服务保持原状；新增专项为只读，写入回归仅使用8016隔离副本。
...
**当前进度**：排名页改为单表单滚动参照、整体不透明sticky双层表头与冻结身份列；指标列128px、状态列192px，数值和说明在列内换行。错误全文移到“原因”弹窗，部分成功保留真实状态，标题改为“车型平台项”避免把81项当81款车型。易车注册为 `yiche-reputation-url-v2`，后台读取已验证URL，无ADB/Runtime/截图调用；会话可选但URL身份仍校验。正式/补跑/映射/基线按平台策略计证据，新三平台81项正常截图分母54；旧映射继续按哈希有效，历史终态不重写。当前模式由capabilities发布，旧原生错误展示为“历史失败/旧流程”，原文按需可查；映射页不生成无图的截图入口。
**后端证据**：`python -m unittest tests.test_backend tests.test_dashboard tests.test_reputation tests.test_reputation_yiche_url -q` 共141项通过。新增5项URL模式测试覆盖零截图/无Runtime、空评分及合法零点评、身份错误不掩盖、平台策略隔离；三平台正式执行测试核对81结果/54证据。真实已验证风云A9 URL单项约3031ms成功，评分/口碑量保持null，排名17、点评76，PNG数0；详见 `artifacts/runtime/reputation-url-fix/url-probe.json`，样本数据未回写历史批次。
**UI证据**：TypeScript、双构建、Ruff、`git diff --check`通过；全站25项浏览器回归通过。`python scripts/verify-reputation-ranking.py` 对用户同一 `RP-S-20260907-FCE5` 批次执行11项检查，覆盖2560/1440/1024宽度、深浅色、起点/中段/最右3个横滚位置及纵滚；所有指标/状态列头与正文x误差≤1px，冻结列和表头命中检查通过、无单元格溢出、无页面JS异常。历史完整API响应前后SHA-256一致。报告与已目视核验PNG位于 `artifacts/runtime/reputation-url-fix/ranking-verification.json`、`ranking-*.png`。
**导出补充**：基线汇报按执行项实际证据要求显示分母；URL-only项明确标注“未要求截图”。XLSX派生预览同样使用中性“本项未要求截图”，策略写入派生图输入哈希，避免复用旧“缺图”占位图。页面不把0/0证据误报为已经生成。
**运行与边界**：本机5173/8000已生效。Git忽略的 `H:\ThreadSnap\artifacts\runtime\tactile-live\serve-home.py` 仅对口碑加载本次规范服务/注册表/URL适配器，普通帖子采集继续用原工作树；原20项未提交采集研发改动仍保留。旧批次仍保留原失败状态和证据要求，未自动重跑或篡改历史。ADR 0070记录临时模式与恢复条件。
**下一步**：完成PR合并，用户刷新当前批次页即可查看；后续计划及显式失败项补跑使用新的URL模式。
**关联**：实现提交 `0fe9c78`、`ffddcf8`；[PR #279](https://github.com/olelius/thread-snap/pull/279)。
...
**干到哪里了**：
- [x] 将排名表拆为独立表头与可滚动正文，纵向滚动不再让正文进入表头区域；正文滚动事件同步表头横向位置。
- [x] 两张表共用显式 `<colgroup>` 与固定总宽度，平台指标列按 112px、状态/证据列按 128px 对齐；左侧冻结列改用不透明背景，横向滚动不再透出底层数据。
- [x] 真实批次 `RP-S-20260904-13E6` 首屏、纵向滚动和横向滚动截图均确认表头、正文和冻结列层级稳定。
**验证**：`frontend\npm run check`、`frontend\npm run build`、`git diff --check`通过；Playwright 实测表头/正文表宽均为 1632px、横向滚动位置同步为 266px；`artifacts/runtime/ui-ranking-split-final-top.png`、`ui-ranking-split-final-scrolled-left.png`、`ui-ranking-split-final-horizontal.png` 已人工核验。
**边界**：仅调整排名表渲染结构、列宽和滚动同步，不改变巡检批次数据、指标语义或证据文件。
...
**干到哪里了**：
- [x] 两级表头单元格分别固定在 `top-0` 与 `top-10`，显式使用不透明卡片背景和高于冻结数据列的层级；平台分组表头、指标表头和角色/车型冻结表头不再让正文透出。
- [x] 真实批次 `RP-S-20260904-13E6` 首屏及纵向滚动截图均确认表头完整、首行不再覆盖表头，左侧冻结列仍可读。
**验证**：`frontend\npm run check`、`frontend\npm run build`、`git diff --check`通过；渲染产物 `artifacts/runtime/ui-ranking-header-fixed-live.png` 与 `artifacts/runtime/ui-ranking-header-fixed-scrolled.png` 已人工核验。
**边界**：仅调整排名表头定位、背景和层级，不改变批次数据、指标语义或证据文件。
**关联**：`frontend/src/features/reputation/reputation-detail-page.tsx`
...
## 2026-09-03 — 汽车之家与易车口碑巡检接入
**总目标**：在隔离工作树中让汽车之家和易车复用懂车帝的映射、正式巡检、截图、汇报和XLSX闭环，并保留平台真实指标语义与历史范围。
**状态**：✅ 代码、真实代表样本、完整验证、PR合并及合并后核验均已完成。
**干到哪里了**：
- [x] 功能提交`f3c7f12`经PR #265合并为`main@a03adc1`；合并基线已包含并保留PR #263/#264的易车页面证据修改，未回退其他工作线。
- [x] 从`main@9406ae7`的独立工作树`H:\ThreadSnap-worktrees\autohome-yiche-reputation`开发，已包含并保留PR #263/#264合入的易车圈子页面证据成果，没有修改其他智能体工作树。
- [x] 新增口碑平台注册表、平台中立适配器合同和共用浏览器骨架；汽车之家、易车均复用单平台草稿/验证/显式发布、固定并发2、90秒单项/一次重试/45分钟整批、逐项持久化、前日比较、全量截图、补跑、平台分组汇报和证据包。
- [x] 范围发布按实际平台生成车型笛卡尔积；完整三平台自动化批次为81个唯一执行项、81份证据。排名页保持一款车型一行、每平台五指标与状态/证据六列组；XLSX为每平台五指标加单一备注，27行各嵌入一张带三平台名的派生预览并保存来源及预览哈希。
- [x] 汽车之家真实艾瑞泽8样本`https://k.autohome.com.cn/6664/`取得4.43分、紧凑型车口碑评分排行21、86人、1097篇；1240×1252区域图SHA-256为`6e978dd2123c73ec5f15707bed8d3daca909f58be04e8684d21845fc97c245d2`，人工确认车型、页面指标和1097条口碑完整且无遮挡。
- [x] 易车真实瑞虎8样本`https://dianping.yiche.com/ruihu8/koubei/`以`serialId=5313`取得4.65分、中型SUV指数排行17、424人、344篇；1240×287区域图SHA-256为`f7481968d5df19698b7939de8e2d9c56bcd680ffba9c446808b8f189e04f5c60`，人工确认身份、排名、分数、人数和标签区域完整且无遮挡。
- [x] 27款候选已生成但保持Git外：汽车之家14个同名、12个唯一非同名、1个无候选；易车25个同名、2个聚合车系歧义。两平台差评率没有可靠平台分子、分母，保存`unknown`并使执行项部分成功，不用标签提及占比、空过滤结果或零值补造。
- [x] 验证：口碑25项和完整后端275项通过；Ruff、compileall、pip check、`git diff --check`通过；前端TypeScript与2468模块生产构建通过。Git外总回执`artifacts/runtime/reputation-autohome-yiche-20260903/acceptance-summary.md`的SHA-256为`66ec7efca18067e796ab5b864b3ebdb74e7b1f4da10111f9087f814da041ae1a`。
**下一步**：本任务已关闭；后续仅在业务方确认候选映射后显式发布汽车之家/易车生产映射，若平台出现可复核的差评分子与分母来源，再按新证据扩充差评率口径。
**边界**：适配器与公共闭环完成不等于54项候选已成为生产映射。候选仍需人工确认、真实验证和显式发布；现有已发布生产范围仍是懂车帝27项，只有完整三平台新范围才是81项，既有历史不扩张。
**关联**：`docs/adr/0064-register-autohome-and-yiche-reputation-adapters.md`、`docs/chains/reputation-inspection.md`、`docs/chains/later-platform-delivery.md`
...
- [x] 既有恢复合同保持不变：到期只领取同平台FIFO队首的一个来源，以实际并发1复访原URL；仍为429则写入下一个10秒时间点，成功则继续原任务。
- [x] 新增定向回归，连续4次易车429的重试次数为`1,2,3,4`且每次下次时间均为约10秒；通用平台递增冷却和“有进展重置、零进展升级”回归同时通过，共3项。
...
**状态**：✅ 状态机修正、真实并发4验收、完整质量门、Git交付及合并后运行核验均已完成。
**干到哪里了**：
- [x] 复核早期A/B：四组新传输访问的是4个不同未解决URL，不是同一触发URL；因此该证据只排除账号和全局出口整体封锁，不支持“同URL换传输必定恢复”。
- [x] 同范围并发4基线`20260902-133622-001`在119条与207条两次限流，冷却60+120秒，最终222条、失败0、服务端耗时249.811秒。
- [x] 全池传输重置反证批次仍冷却60+120秒，最终222条、失败0、耗时258.950秒；固定0.5秒详情间隔反证批次同样冷却两次，耗时312.342秒。两条路线均已撤销，没有进入最终代码。
- [x] Worker检查点新增`rate_limit_completed_count`：本次429前已经推进完成数时将`retry_attempt`重置为1；完成数未推进的连续429才按120、240、480、900秒升级。
- [x] 定向3项通过，覆盖首次60秒、有进展后再次60秒、零进展后升级120秒，以及既有认证后限流和多来源FIFO单并发恢复。
- [x] 最终真实并发4批次`20260902-144417-001`的8个任务均冻结并发4；在116条时只冷却60秒，恢复后新增106条并直接完成，终态222条、失败0、8/8来源成功，服务端耗时124.193秒、吞吐1.788条/秒。较旧并发4基线缩短50.3%，较同范围并发1缩短56.7%；平台运行配置已恢复1。
- [x] Git外验收回执`artifacts/runtime/yiche-429-progressive-cooldown/concurrency4.json`的SHA-256为`eab7064ea212c149f0ca06eddbbcf9bb239a6cc864adfaf0e60df6168496e3c8`。
- [x] 新增ADR 0062，并同步领域词汇、产品设计、技术路线、ADR 0052/0058、文档索引和后续平台链。
- [x] 完整后端267项通过；Ruff、compileall、pip check、`git diff --check`、前端TypeScript检查和2468模块生产构建通过。
- [x] 提交`9a1a66b`经PR #258合并为`main@3ae0def`，远端与本地功能分支均已清理；最终main重启后的运行核验通过。
**下一步**：本任务已关闭；运行配置保持并发1。后续按真实业务批次继续观察“有进展重置、零进展升级”的分布，不把单轮124.193秒外推为固定耗时。
**边界**：本轮修正优化的是冷却状态机，不宣称平台免控；仅当同一任务完成数确有推进时重置级别，冷却后零进展再次429仍按原阶梯升级；当前运行配置保持1。
**关联**：`docs/adr/0062-reset-rate-limit-backoff-after-confirmed-progress.md`、`docs/adr/0052-resume-rate-limited-post-tasks-after-cooldown.md`、`src/threadsnap/worker.py`
...
- [x] 提交`bbf3f93`经PR #255合并为`main@39c6f98`，远程与本地功能分支均已清理；合并后接口确认易车为v9且当前配置并发1。
- [x] 同范围8来源、目标240的参考并发1批次`20260902-114803-001`以222条、失败0成功结束，服务端耗时287.123秒、吞吐0.773条/秒。
- [x] 实际并发4批次`20260902-124831-001`在12.177秒完成59条、14.188秒时因随机TDC变体进入等待；批次8个任务快照均为4，平台配置随后恢复1。初始计时回执为`artifacts/runtime/yiche-429-transport-rotation/concurrency4-task.json`，SHA-256为`81878d89eea3d5b2b8e850fed47f6ab06aa6ebb8e62f72cbf80480fc7dc1d4ef`。
...
## 2026-09-01 — 易车并发1固定1000 URL控制测试
**总目标**：以易车v7、同一加密Session和实际并发1访问事前冻结的1000个不同详情URL，确认单并发是否仍出现平台控制，并验证原批次恢复不重复已完成请求。
**状态**：✅ 单并发控制结论、URL清单恢复修复、真实1000条原批次终态、根因对照和Git交付均已完成。
**干到哪里了**：
- [x] 从8个已验证来源按轮询分层冻结1000个不同详情URL，输入SHA-256为`e8a5437938d85baeefd626b82bfe541e3983bd72353072abb4e4f59ef8570d67`；发现阶段使用并发1，账号和列表访问成功。
- [x] 批次`20260901-164445-001`关闭AI和截图、快照并发1；先后在完成数59、118、840、897、954观察到5次`PLATFORM_CAPTCHA_REQUIRED`，并在118到840之间观察到7次详情页面HTTP 429冷却边界。原批次最终于18:21:46完成`1000 / 1000`、最终失败0，1000个原位置完整且唯一。
- [x] 运行检查发现当前URL清单控制恢复只有59个已完成ID，没有剩余URL检查点；旧Worker恢复会再次传入完整1000 URL。已实现领取时按持久化帖子ID过滤，并从原清单建立稳定位置映射，目标是只续跑剩余941条。
- [x] 新增回归证明3条URL在首条持久化后进入访问验证，恢复只请求后2条且最终顺序保持0/1/2；完整后端252项、Ruff、compileall、pip check、前端TypeScript检查和2468模块生产构建均通过，`git diff --check`通过。
- [x] 修复提交`a1836d1`经PR #246合并为`main@7350693`，远程与本地功能分支均已清理；合并后服务重启且`/health=ok`，后续五次验证码、七次限流冷却和最终1000/1000均由同一原任务续跑，没有创建替代批次或重复已完成URL。
- [x] 限流A/B证明生产长会话处于429冷却时，同出口的新HTTP会话在空Cookie、设备Cookie、账号Cookie和完整Session四种状态下均可取得正文；验证码期间账号身份仍通过、列表API仍返回50条，而同出口、同保存Session的全新HTTP传输仍取得`203 -> 200(WAF CAPTCHA)`。因此账号退出和429的全局IP封禁均被排除；验证码更符合详情路由的共享复合风险桶。
- [x] 第3、4、5次验证码均在约13分钟静默后，以同IP、同账号、同Profile、同触发URL且零滑块交互直接恢复正文；恢复后两个无并行探针干扰的周期均新增57条再次触发，最后46条完成。并发1只串行而没有限速，本轮57～59条可在约17～20秒内形成。
- [x] 修复回执`acceptance.json`的SHA-256为`a617e3619ae1e77039c065c4b30b4a469aed7a455b1128c386991b8586b36c51`；脱敏根因报告`root-cause-analysis.json`的SHA-256为`3737de8252999f97d1c7a9a4b577181d8d4fa26df78100997ff334486a35d800`；输入清单SHA-256为`e8a5437938d85baeefd626b82bfe541e3983bd72353072abb4e4f59ef8570d67`。
**下一步**：正式500条验收前，先用独立冻结输入验证显式请求节流而非只把并发设为1；若仍需量化IP权重，再准备不改动当前主机网络的独立第二出口做同账号、同URL、同一时点A/B。
**边界**：本轮不把约13分钟写成平台精确TTL，不把57～59条写成服务端固定阈值，不把本机单出口证据外推为IP无关；不提高易车并发，不把诊断1000条替代正式500条分母，完整URL和原始结果只保存在Git外`artifacts/poc/`。
**关联**：`src/threadsnap/worker.py`、`tests/test_backend.py`、`docs/adr/0058-route-yiche-control-through-classified-recovery.md`、`docs/design/product-design.md`、`docs/design/technical-route.md`
...
**干到哪里了**：
- [x] 现场数据库确认月末批次 `RP-S-20260831-A7BC` 于10:00以并发8执行27项并在约45秒内27/27成功；前一日16:35左右懂车帝帖子配置已从既有2调整为8，旧巡检创建逻辑直接继承该值，因此不是窗口泄漏，而是两类业务并发意外耦合。
- [x] 正式日检、月末、同日延迟巡检及失败项补跑统一使用后端常量2；新批次快照写入2，执行入口把修复前遗留或异常写入的非终态快照收敛为2，终态历史记录不改写。
- [x] 新增平台并发8与异常排队快照8的回归，证明适配器实际接收2、批次快照回到2，补跑也保存2；目标口碑测试22/22、完整后端223/223、Ruff、compileall、pip check及`git diff --check`通过。
- [x] 本地后端已重启加载修复，`/health=ok`、运行常量为2；懂车帝帖子提取仍保持用户配置8，数据库`PRAGMA integrity_check=ok`，历史月末批次仍保持并发8。脱敏回执位于`artifacts/runtime/reputation-concurrency-two-20260831/summary.json`。
- [x] 修复已由PR #240交付；功能分支在合并后清理，本地与远程`main`同步。
**下一步**：观察下一个正式或失败项补跑批次冻结并显示并发2；不需要修改懂车帝帖子提取当前并发8。
**边界**：映射验证不是巡检批次，继续使用既有平台并发快照；历史终态批次、指标、证据和汇报保持不可变；帖子提取1至8并发配置不被本修复改写。
**关联**：`docs/adr/0053-fix-reputation-run-concurrency-at-two.md`、`src/threadsnap/reputation.py`、`tests/test_reputation.py`

---

## 2026-08-30 — 平台限流在原帖子任务冷却续跑
**总目标**：消除汽车之家暂时限流被终态化后反复创建补提批次的问题，同时保留原固定候选、来源位置和平台FIFO止损边界。
**状态**：✅ 代码、固定候选、冷却状态机、owner文档、完整验证、本地服务加载和Git交付均已完成。
**干到哪里了**：
- [x] 只读数据库复核历史链：批次 `20260830-185206-001` 的7个来源在冻结并发8下同时启动，3个来源于约49～73秒内被旧逻辑以限流终态结束，另外4个来源经会话恢复后完成；后续 `20260830-220226-001` 仍以并发8在约3秒完成剩余42条，证明并发8不是单一充分原因，重复补提来自限流终态化。
- [x] 共享Worker把 `PLATFORM_RATE_LIMITED` 改为同一圈子任务的持久冷却检查点：60秒起指数退避、最高900秒；到期只领取FIFO队首一个限流来源并把实际请求并发收敛为1，成功后才处理下一个来源，不增加失败数或创建补提批次。
- [x] 汽车之家升级为 `autohome-club-v6`，先冻结本轮全部候选再访问详情；详情或已知URL限流后停止后续请求，并把当前及剩余固定URL与来源位置交给Worker原位续跑，不重新读取列表补位。
- [x] 新增会话刷新后限流原任务续跑、多个限流来源逐项单并发恢复、圈子固定剩余候选和URL清单剩余项保留回归；6项直接相关测试及既有网络重试回归通过。
- [x] 完整后端221/221及新增退避序列专项1/1通过，Ruff、compileall、pip check、前端TypeScript检查、2468模块生产构建和 `git diff --check` 通过。
- [x] 本地后端已重启加载新代码，`/health=ok`，平台接口返回汽车之家 `autohome-club-v6`、`available`；前端5173返回HTTP 200。脱敏回执位于 `artifacts/runtime/rate-limit-cooldown-20260830/summary.json`。
- [x] 修复已由 PR #239 交付；后续真实限流批次将保留在原任务冷却续跑，历史终态批次不迁移。
**下一步**：以新批次观察真实限流时的60秒起原位冷却和自动恢复；当前实现不再需要人工创建“重新提取失败项”批次。
**边界**：验证码和访问验证继续走 `waiting_for_auth`；限流不冒充登录失效，不立即强刷；正式500验收执行器仍服从独立固定窗口，不跨窗口拼接结果；历史终态批次不回写。
**关联**：`docs/adr/0052-resume-rate-limited-post-tasks-after-cooldown.md`、`src/threadsnap/worker.py`、`src/threadsnap/collectors/autohome.py`

---

## 2026-08-30 — 汽车之家验证码完成后自动续跑
**总目标**：纠正验证码/访问验证被错误当成重新登录的恢复路径，并在人工验证完成后自动保存更新状态、恢复原任务。
**状态**：✅ 代码、状态路由、自动续跑、错误分类、owner 文档、完整验证、本地服务加载和Git收尾均已完成。
**干到哪里了**：
...
- [x] 修复已由 PR #237 合并为 `main@a20acde`，远程和本地功能分支均已删除；合并后的本地服务继续返回 `/health=ok`。
**下一步**：当前历史补提批次已在旧逻辑下把正式Profile覆盖为匿名状态，需要一次重新登录取得服务器Session；之后新发生的验证码恢复将复用登录Profile并在验证完成后自动续跑。
