...
**状态**：⏳ 已在独立分支 `fix/reputation-validation-selection` 完成前端目标筛选与缺失映射提示；后端受影响回归 2 项、前端类型检查、生产构建和 5 项前端测试通过。最小更新包已构建并通过包内校验；后续需完成真实线上资源/接口核验、提交推送合并及服务器切换与回滚核验。
...
**下一步**：使用 `PYTHONPATH=H:/ThreadSnap-reputation-validation-fix/src` 执行受影响后端回归；构建最小前端更新包，复用服务器依赖与当前/previous 原子切换流程，部署后确认 `/health`、前端资源、发布预览和验证入口行为。
