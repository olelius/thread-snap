...
**状态**：🟡 v0.2.9 已打通两个候选的验证码按钮点击但手机均未收到；正在生成点击后平台响应证据包
...
**下一步**：生成并覆盖新的免重装包后，分别执行候选 A/B 至 `sms_send_evidence`；依据两者的请求路径、HTTP 状态、倒计时和可见提示，判定事件触发、平台接受、附加验证或短信投递层。取得动态码并保存两份会话后复跑联通门；只有复核为 `ready_for_2000=true` 后才运行首轮 2000 条。
