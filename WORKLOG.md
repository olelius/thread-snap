...
**总目标**：把第一版正式登录方式从“数据库明文保存平台账号密码”调整为“客户机远程操作服务器官方登录页面、加密 Session、有界自动刷新、认证后自动续跑原批次”，同时保持当前懂车帝单平台基础闭环范围。
...
- [x] 浏览器进程、Profile 和 Session 位于服务器；客户从客户机通过 ThreadSnap 或客户现有系统的临时、已鉴权远程交互入口操作服务器浏览器中的平台官方页面，普通外链和客户机本地登录状态不作为服务器 Session。
...
- [x] 文档一致性检查确认六份 owner/账本文档均包含服务器浏览器、客户机操作、`waiting_for_auth` 和自动续跑口径，未发现旧的失败终态/人工重试表述；项目 `.vevn` 的64项 `poc/shared` 测试及 `git diff --check` 通过。

**下一步**：正式功能开发时先落地平台 Session 状态模型、加密存储边界、`waiting_for_auth` 批次检查点和自动续跑状态机，再接入客户机到服务器浏览器的临时鉴权交互组件；具体远程传输实现随最终前后端与部署技术栈选择。

**边界**：本次只确定第一版平台认证与 Session 生命周期，不新增多账号、账号轮换、自动接码、验证码识别或长期浏览器托管，也不扩展当前第一版业务范围。等待认证是持久化批次状态，不长期占用采集进程、数据库事务或公开浏览器控制端口。
