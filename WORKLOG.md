...
**状态**：🟡 目标 Linux 环境与浏览器路径已通过；已确认默认手机验证码模式未正确切到密码登录，修复中
...
**下一步**：生成密码登录模式修复包；目标服务器覆盖两个候选入口后复跑联通脚本，把新的诊断 `tar.gz` 和 `.sha256` 复制回来。只有复核为 `ready_for_2000=true` 后才运行首轮 2000 条。
