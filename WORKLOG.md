...
**状态**：🟡 v0.2.8 已证明页面停止加载不释放框架生命周期等待；正在生成直接使用 DOM 就绪条件的修复包
...
**下一步**：生成并覆盖新的免重装包后，先执行候选 A 短信初始化；确认 `navigation_action=...wait_until_domcontentloaded` 后应直接出现 `sms_page_ready` 与 `sms_request_clicked`。候选 A 成功后再执行候选 B并复跑联通门；只有复核为 `ready_for_2000=true` 后才运行首轮 2000 条。
