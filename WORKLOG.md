...
**状态**：⏳ 独立分支 `fix/reputation-validation-stale-contract` 已加入当前平台合同识别；映射完整且合同过期的项会进入验证目标，缺少映射项继续跳过并单独提示。前端类型检查、6 项前端测试和既有生产构建通过，待提交、合并和服务器更新。
...
**下一步**：执行前端检查/构建与定向测试，提交 PR 后按最小更新包发布，核对易车按钮显示可重新验证数量和线上接口健康。
